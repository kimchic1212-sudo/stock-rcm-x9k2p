...
async function uploadDebugImage(name, buf) {
  try {
    const path = `/repos/${CONFIG.ghOwner}/${CONFIG.ghRepo}/contents/debug/${name}`;
    const cur = await ghRequest('GET', path).catch(() => ({ status: 404 }));
    const sha = cur.status === 200 ? cur.body.sha : undefined;
  ...
  } catch(e) { log(`스크린샷 업로드 실패: ${e.message}`); }
}
...
async function fetchPOSSales() {
  ...
  const itemPromises = [];
  let dialogMsg = null;
  // 디버그 자료(스크린샷·로그)는 메모리에만 모아 두고 문제가 있을 때만 올린다.
  // 예전엔 성공할 때도 매번 올려 10분마다 커밋 3개씩 쌓였고 데이터 저장소가 1GB까지 커졌음(2026-09-22).
  const shots = [];
  let problem = '';
  const snap = async (name, page) => {
    try { shots.push([name, await page.screenshot({ fullPage: false })]); }
    catch(e) { debugLog.push(`스크린샷 실패: ${e.message}`); }
  };
  ...
  try {
    ...
    page.on('response', response => {
      ...
      if (url.includes('selTodaySalesList')) {
        ...
        }).catch(e => { debugLog.push(`selTodaySalesList json 실패: ${e.message}`); problem = problem || 'POS 응답 해석 실패'; });
        itemPromises.push(p);
      }
      if (url.includes('selItemSalesList')) {
        ...
        }).catch(e => { debugLog.push(`selItemSalesList json 실패: ${e.message}`); problem = problem || 'POS 응답 해석 실패'; });
        itemPromises.push(p);
      }
    });
    ...
    // 스크린샷 (로그인 직후) — 문제가 생긴 경우에만 올라감
    await snap('after_login.png', page);
    ...
    if (tradeNos.length === 0) {
      ...
      for (const frame of page.frames()) {
        ...
        if (res) { menuClicked = true; debugLog.push(`[${frame.url().split('/').pop().slice(0,20)}] ${res}`); }
      }

      // 추가 대기
      await page.waitForTimeout(10000);
      await snap('after_menu_click.png', page);
      debugLog.push(`메뉴 클릭 후 거래: ${tradeNos.length}건`);
      // 판매가 아직 없는 시간(영업 전)에는 거래 0건이 정상 — 메뉴 자체를 못 찾았을 때만 문제로 본다
      if (!menuClicked) problem = problem || '당일매출조회 메뉴를 못 찾음';
    }
    ...
    if (tradeNos.length > 0 && Object.keys(todayItems).length === 0) problem = problem || '거래는 있는데 품번을 하나도 못 읽음';
    log(`수집 완료: ${Object.keys(todayItems).length}개 품번`);

  } catch(e) {
    problem = problem || `오류: ${e.message}`;
    throw e;
  } finally {
    if (problem) {
      debugLog.push(`문제: ${problem} → 디버그 자료 업로드`);
      for (const [name, buf] of shots) await uploadDebugImage(name, buf);
...
(async () => {
  ...
  try {
    ...
    // 로그인 성공 — 실패 카운터를 리셋하고, 판매가 없어도 '정상 확인' 시각(lastCheckedAt)을 남긴다.
    // 앱이 영업 전·판매 없는 시간에 '미갱신' 경고를 띄우지 않게 하려는 것. 커밋이 너무 잦지 않도록
    // 최대 1시간에 1번, 그리고 한국 날짜가 바뀐 뒤 첫 실행에서 기록한다.
    const needsRecover = (status.consecutiveFailures || 0) > 0 || status.locked;
    const lastChecked = Date.parse(status.lastCheckedAt || '') || 0;
    const kstDay = t => new Date(t + 9 * 3600 * 1000).toISOString().slice(0, 10);
    if (needsRecover || Date.now() - lastChecked > 55 * 60 * 1000 || kstDay(lastChecked) !== kstDay(Date.now())) {
      const next = needsRecover ? { consecutiveFailures: 0, locked: false, recoveredAt: new Date().toISOString() } : { ...status };
      next.lastCheckedAt = new Date().toISOString();
      await saveSyncStatus(next, statusSha);
    }
