...
function rebuildIndex(){
  const map = new Map();
  const prevRaw = JSON.parse(localStorage.getItem('PREV_RAW') || '[]'); // 전일 데이터
  ...
  for(const r of RAW){
    ...
    if(!map.has(code)){
      map.set(code, {
        ...
        소비자가:Number(r["소비자가"]||0), shopNo:"", sizes:[]
      });
    }
...
function applyFilters(){
  ...
  if(tokens.length > 0 && !RECENT_SEARCHES.includes(f.q)) {
...
function card(p){
  ...
  el.className = "card card-hover p-4 flex flex-col gap-3 relative";
  ...
  const deltaBadge = p.delta > 0 ? `<span class="badge delta-badge" style="background:#ECFDF5;color:#047857">▲ 입고 +${p.delta}</span>` : 
                    (p.delta < 0 ? `<span class="badge delta-badge" style="background:#FFF1F2;color:#E11D48">▼ 판매 ${p.delta}</span>` : '');
  const urgentBadge = p.urgentRestock ? `<span class="badge badge-last urgent-badge" style="border:1px solid red">🚨 황금결품</span>` : '';
  const favIcon = FAVS.includes(p.품번) ? '★' : '☆';

  const top = document.createElement("div");
  top.className = "flex items-center gap-1.5 flex-wrap";
  top.innerHTML = `<span class="badge badge-cat">${escapeHtml(p.카테고리||"-")}</span>
                   <span class="badge badge-brand">${escapeHtml(p.브랜드||"-")}</span>
                   <span class="badge ${g.cls}">${g.sym} ${g.label}</span>
                   ${deltaBadge} ${urgentBadge}`;
  el.appendChild(top);

  // 즐겨찾기 버튼
  const favBtn = document.createElement("button");
  favBtn.className = "fav-btn absolute top-4 right-4 text-lg";
  ...
  titleRow.innerHTML = `<div class="flex flex-col gap-1 items-start mt-1">
      <button type="button" class="copyable text-left" data-copy="${escapeHtml(p.품명)}">${escapeHtml(p.품명||"-")}</button>
      <button type="button" class="copyable mono text-sm text-[color:var(--muted)] border-none px-0" data-copy="${escapeHtml(p.품번)}">${escapeHtml(p.품번||"-")}</button>
  </div>`;
  el.appendChild(titleRow);

  const sizes = document.createElement("div");
  sizes.className = "grid gap-1.5 mt-2";
  sizes.style.gridTemplateColumns = "repeat(auto-fill, minmax(56px, 1fr))";
  for(const s of p.sizes){
    const q = s.busan||0;
    let cls = "size-cell tnum ";
    if(q===0) cls+="zero"; else if(q===1) cls+="last";
    sizes.innerHTML += `<div class="${cls}"><span class="sz mono">${escapeHtml(String(s.size))}</span><span class="qty mono">${q}</span></div>`;
  }
  el.appendChild(sizes);

  const loc = document.createElement("div");
  loc.className = "loc mono tnum mt-auto";
  loc.innerHTML = `<span class="loc-item ${p.busanTotal>0?'has':'zero'}"><span class="dot"></span>부산 ${p.busanTotal}</span>
                   <span class="loc-item ${p.sinsaTotal>0?'has':'zero'}"><span class="dot"></span>신사 ${p.sinsaTotal}</span>
                   <span class="loc-item ${p.centerTotal>0?'has':'zero'}"><span class="dot"></span>물류 ${p.centerTotal}</span>
                   <span class="ml-auto font-extrabold text-[color:var(--ink)]">${krw(p.소비자가)}</span>`;
  el.appendChild(loc);
  return el;
}
...
function openDetail(p){
  ...
  const rows = p.sizes.map(s=>`
    <tr class="mono tnum text-sm border-t border-[color:var(--line)]">
        ...
        <td class="text-center px-2 font-extrabold ${s.busan>0?'text-green-700':'text-gray-400'}">${s.busan||0}</td>
        <td class="text-center px-2">${s.sinsa||0}</td>
        <td class="text-center px-2">${s.center||0}</td>
    </tr>`).join("");

  $("#detailBody").innerHTML = `<table class="w-full mt-4 bg-[color:var(--bg)] rounded-lg"><thead>${head}</thead><tbody>${rows}</tbody></table>`;
  ...
  // ADMIN 전용 자사몰 이미지 오토스크랩 툴
  if (sessionStorage.getItem(SESSION_FLAG) === "1" && p.shopNo) {
      const adminImgBox = document.createElement("div");
      adminImgBox.className = "mt-4 p-3 rounded-lg border-2 border-dashed border-black bg-gray-50";
      
      const currentImgMsg = (typeof IMAGES !== "undefined" && IMAGES[p.shopNo]) 
          ? `<div class="text-[11px] text-blue-600 font-bold mb-2">※ 현재 이 상품은 이미지가 등록되어 있습니다.</div>` : ``;

      adminImgBox.innerHTML = `
          <div class="flex items-center justify-between mb-2">
            <div class="text-xs font-bold text-gray-600">👑 점장(ADMIN) 이미지 관리</div>
            <button id="autoFetchBtn" class="px-2 py-1.5 text-[11px] font-black bg-green-600 text-white rounded">자사몰 오토스크랩</button>
          </div>
          ${currentImgMsg}
      ...
      adminImgBox.querySelector("#autoFetchBtn").addEventListener("click", async () => {
          ...
          try {
              ...
              if (match && match[1]) {
                  ...
                  msg.style.color = "red"; msg.textContent = "실패. 열리는 창에서 직접 복사하세요."; window.open(targetUrl, "_blank"); 
              }
...
$("#addCartBtn").addEventListener("click", () => {
    ...
    CART.push({ 품명: CURRENT_PRODUCT.품명, 사이즈: size, 수량: qty });
...
$("#copyCartBtn").addEventListener("click", () => {
    const text = CART.map((c, i) => `${i+1}. ${c.품명} (${c.사이즈}) - ${c.수량}개`).join("\n");
    copyText(`[해운대점 이동요청]\n${text}`, $("#copyCartBtn"));
    CART = []; localStorage.removeItem('CART'); updateCartCopyBtn(); // 복사 후 비우기
});
...
$("#file").addEventListener("change", e=>{ 
    ...
    localStorage.setItem('PREV_RAW', JSON.stringify(RAW)); // 전일 데이터 저장
