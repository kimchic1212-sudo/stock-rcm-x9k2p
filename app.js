// 🔥 1. CSS 충돌 원천 해결: hidden 상태일 때는 flex가 적용되지 않도록 수정 🔥
const style = document.createElement('style');
style.innerHTML = `
    .modal-backdrop:not(.hidden) { display: flex !important; align-items: center; justify-content: center; padding: 15px; }
    .modal-content { 
...
function applyMeta(meta){
    if(meta) {
        const el = $("#statSrc");
        if(el) {
            let addInfo = SALES_HISTORY.meta?.name ? `<div class="text-[11px] text-orange-600 mt-0.5 font-bold">📊 연동됨: ${escapeHtml(SALES_HISTORY.meta.name)}</div>` : "";
  ...
  if(!$("#salesPeriodWrap") && $("#sortSel")) {
      ...
      wrap.innerHTML = `
          ...
          <div id="customDateWrap" class="hidden items-center gap-1 bg-white p-1 border border-orange-200 rounded">
              <input type="date" id="customStartDate" class="ipt text-[11px] px-1 py-1 w-[90px] border-none"> ~ <input type="date" id="customEndDate" class="ipt text-[11px] px-1 py-1 w-[90px] border-none">
              <button id="customDateApply" class="px-2 py-1 bg-orange-500 text-white rounded text-[11px] font-bold">적용</button>
          </div>
          <button id="openAnalyticsBtn" class="hidden px-2 py-1.5 bg-gray-800 text-white text-[12px] font-black rounded flex items-center gap-1 shadow-sm shrink-0"><i data-lucide="pie-chart" class="w-3.5 h-3.5"></i> 리포트</button>
      `;
      ...
      $("#customDateApply").onclick = () => { if(!$("#customStartDate").value || !$("#customEndDate").value) return; $("#openAnalyticsBtn").classList.remove("hidden"); visibleCount=60; render(); };
    ...
    modal.innerHTML = `
        ...
        <div class="modal-content relative bg-[#f8fafc] w-[90%] max-w-md flex flex-col rounded-2xl shadow-2xl z-10 border border-gray-200">
            ...
            <div class="p-3 bg-white border-b border-gray-200 flex gap-2 z-10 shadow-sm">
                <select id="dashCatSel" class="ipt flex-1 text-xs font-bold bg-gray-50 border-gray-200 rounded"><option value="ALL">📦 전체 카테고리</option><option value="신발">👟 신발만</option><option value="의류">👕 의류만</option><option value="용품">🎒 용품만</option></select>
                <select id="dashBrandSel" class="ipt flex-1 text-xs font-bold bg-gray-50 border-gray-200 rounded"><option value="ALL">🏷️ 전체 브랜드</option>${brands.map(b => `<option value="${escapeHtml(b)}">${escapeHtml(b)}</option>`).join('')}</select>
            </div>
            <div id="dashBody" class="p-5 overflow-y-auto space-y-5"></div>
        </div>`;
    const renderDashBody = () => {
        const catF = $("#dashCatSel").value; const brF = $("#dashBrandSel").value;
        let total = 0; let catD = { "신발":0, "의류":0, "용품":0 }; let genD = { "M":0, "W":0, "U":0 }; let brD = {}; let sold = [];
        PRODUCTS.forEach(p => {
            if(p.periodSales > 0) {
                if(catF !== "ALL" && p.카테고리 !== catF) return; if(brF !== "ALL" && p.브랜드 !== brF) return;
                total += p.periodSales; catD[p.카테고리||"용품"] = (catD[p.카테고리||"용품"]||0) + p.periodSales;
                genD[p.gender||"U"] = (genD[p.gender||"U"]||0) + p.periodSales; brD[p.브랜드||"기타"] = (brD[p.브랜드||"기타"]||0) + p.periodSales; sold.push(p);
            }
        });
        ...
        const maxC = Math.max(...Object.values(catD), 1); const maxG = Math.max(...Object.values(genD), 1);
...
function openSalesGuide(code) {
    ...
    modal.innerHTML = `
        <div class="modal-outer absolute inset-0 bg-black/60 backdrop-blur-sm" onclick="this.parentNode.classList.add('hidden')"></div>
        <div class="modal-content relative bg-white w-[90%] max-w-md flex flex-col rounded-2xl shadow-2xl z-10 p-5">
            <div class="flex justify-between items-start mb-4"><h2 class="font-black text-lg text-indigo-900">🧠 AI 세일즈 가이드</h2><button onclick="this.closest('#salesGuideModal').classList.add('hidden')" class="p-1"><i data-lucide="x" class="w-6 h-6"></i></button></div>
            <div class="space-y-4">
                <div><div class="text-[11px] font-bold text-indigo-400 mb-1">핵심 키워드</div><div class="flex flex-wrap gap-1">${guide.keywords.map(k=>`<span class="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full text-[11px] font-black border border-indigo-100">#${escapeHtml(k)}</span>`).join('')}</div></div>
                <div><div class="text-[11px] font-bold text-indigo-400 mb-1">제품 특징</div><p class="text-sm font-medium text-gray-800 leading-relaxed">${guide.features}</p></div>
                <div><div class="text-[11px] font-bold text-indigo-400 mb-1">추천 고객</div><p class="text-sm font-medium text-gray-800 leading-relaxed">${guide.target}</p></div>
                <div class="bg-indigo-50 p-3 rounded-lg"><div class="text-[11px] font-bold text-indigo-600 mb-1">🗨️ 실전 응대 멘트</div><p class="text-sm font-black text-indigo-900 italic">"${guide.pitch}"</p></div>
            </div>
        </div>`;
...
function card(p){
  ...
  if (pSel && pSel.value !== "" && p.periodSales > 0) {
      sBadge += `<span class="bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded font-black flex items-center gap-0.5">📈 ${p.periodSales}개</span>`;
      if (p.periodSales >= 3 && p.busanTotal <= 1 && (p.sinsaTotal > 0 || p.centerTotal > 0)) sBadge += `<span class="bg-red-600 text-white px-1.5 py-0.5 rounded font-black animate-pulse">🚨 보충</span>`;
  }
  const guide = SALES_GUIDES[p.품번];
  let sHtml = (guide && guide.keywords) ? `<div class="flex flex-wrap gap-1 mt-1.5 mb-1.5">` + guide.keywords.map(kw => `<span class="bg-indigo-50 text-indigo-600 border border-indigo-100 text-[10px] font-black px-1.5 py-0.5 rounded cursor-pointer hover:bg-indigo-600 hover:text-white transition-colors btn-sales shadow-sm">#${escapeHtml(kw.trim())}</span>`).join('') + `</div>` : "";
  ...
  if (p.currentPromoPrice && p.currentPromoPrice < p.소비자가) {
      ...
      if (p.promoType === 'weekly') { pBadge = `<span class="bg-red-600 text-white px-1.5 py-0.5 rounded font-black flex items-center gap-0.5 shadow-sm">🔥위클리 ${rtL}</span>`; prcDisp = `<div class="flex flex-col items-end leading-tight"><span class="text-[10.5px] text-gray-400 line-through">${krw(p.소비자가)}</span><span class="text-[16px] font-black text-red-600">🔥${krw(p.currentPromoPrice)}</span></div>`; }
      else { pBadge = `<span class="bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded font-black flex items-center gap-0.5 shadow-sm">🎟️쿠폰 ${rtL}</span>`; prcDisp = `<div class="flex flex-col items-end leading-tight"><span class="text-[10.5px] text-gray-400 line-through">${krw(p.소비자가)}</span><span class="text-[15px] font-black text-purple-700">🎟️${krw(p.currentPromoPrice)}</span></div>`; }
  }
  el.innerHTML = `
    <div class="flex justify-between items-start mb-2 z-10 relative">
        <div class="flex flex-wrap gap-1 text-[11px] font-bold text-gray-500 mt-0.5">${busanOnlyBadge}${pBadge}${sBadge}<span class="bg-gray-100 px-1.5 py-0.5 rounded">${escapeHtml(p.카테고리||"-")}</span><span class="bg-gray-100 px-1.5 py-0.5 rounded">${escapeHtml(p.브랜드||"-")}</span><span class="bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded">${escapeHtml(p.성별||p.gender||"-")}</span>${deltaHtml}</div>
    ...
    <div class="loc-simple mt-auto"><div class="flex gap-1 items-center text-[12px]"><b>부산 ${p.busanTotal}</b> | 신사 ${p.sinsaTotal} | 물류 ${p.centerTotal}</div>${prcDisp}</div>`;
...
document.addEventListener("keydown", (e) => {
    if(e.key === "Escape") {
        const modals = Array.from($$('.modal-backdrop:not(.hidden)'));
        if(modals.length > 0) {
...
$$('.modal-backdrop').forEach(m => { m.addEventListener("click", (e) => { if (e.target === m || e.target.classList.contains("modal-outer")) m.classList.add("hidden"); }); });
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    window.renderPromoAdmin = () => {
        const box = $("#promoAdminBox"); if(!box) return;
        if(PROMOTIONS && PROMOTIONS.meta && Object.keys(PROMOTIONS.items || {}).length > 0) {
            box.innerHTML = `<div class="flex justify-between items-center mb-2"><div class="font-black text-purple-800">🎁 기획전 중</div><button id="endPromoBtn" class="px-2 py-1 bg-red-100 text-red-600 text-[10px] font-black rounded">종료</button></div><div class="text-[11px] font-bold text-purple-500">${escapeHtml(PROMOTIONS.meta.name)}</div>`;
            $("#endPromoBtn").onclick = async () => {
                if(!confirm("기획전을 종료하시겠습니까?")) return;
                try {
                    const apiBase = `https://api.github.com/repos/${GH.owner}/${GH.repo}/contents/${PROMOTIONS_PATH}`;
                    let sha = null; try { const r = await fetch(apiBase+"?t="+Date.now(), {headers:{Authorization:"Bearer "+getPat()}}); if(r.ok){ const j=await r.json(); sha=j.sha; } }catch(e){}
                    await fetch(apiBase, { method:"PUT", headers:{ Authorization:"Bearer "+getPat(), "Content-Type":"application/json" }, body: JSON.stringify({ message:"end", content: utf8ToB64(JSON.stringify({}, null, 2)), branch: GH.branch, sha }) });
                    PROMOTIONS = {}; sessionStorage.removeItem(CACHE_KEY); rebuildIndex(); render(); window.renderPromoAdmin(); alert("종료 완료");
                } catch(e) { alert("종료 실패"); }
            }
        } else {
            box.innerHTML = `<div class="text-center cursor-pointer" id="pUTrig"><div class="font-black text-purple-800 text-sm">🎁 위클리특가 등록</div></div><input type="file" id="pFil" class="hidden">`;
            $("#pUTrig").onclick = () => $("#pFil").click();
            $("#pFil").onchange = async (e) => {
                const f = e.target.files[0]; if(!f) return;
                const reader = new FileReader();
                reader.onload = async (ev) => {
                    const wb = XLSX.read(new Uint8Array(ev.target.result), {type:"array"}); const sheet = wb.Sheets[wb.SheetNames[0]]; const rows = XLSX.utils.sheet_to_json(sheet, {header: 1, defval: ""});
                    const pN = String(rows[0][0]||"").replace("기획전명 :", "").trim(); const pP = String(rows[1][0]||"").replace("기간 :", "").trim();
                    let items = {}; let hIdx = rows.findIndex(r => r.includes('품번'));
                    if(hIdx > -1) {
                        const h = rows[hIdx].map(x => String(x||"").trim()); const cI = h.indexOf('품번'), catI = h.indexOf('특가 카테고리'), wI = h.indexOf('위클리특가'), fpI = h.indexOf('최종할인가');
                        for(let i=hIdx+1; i<rows.length; i++) { const r = rows[i]; const c = String(r[cI]||"").trim(); if(c) items[c] = { targetCat: String(r[catI]||"").trim().toUpperCase(), weeklyPrice: Number(String(r[wI]||"").replace(/,/g,'')), finalPrice: Number(String(r[fpI]||"").replace(/,/g,'')) }; }
                    }
                    const nP = { meta: { name: pN, period: pP }, items };
                    try {
                        const apiBase = `https://api.github.com/repos/${GH.owner}/${GH.repo}/contents/${PROMOTIONS_PATH}`;
                        let sha = null; try { const r = await fetch(apiBase+"?t="+Date.now(), {headers:{Authorization:"Bearer "+getPat()}}); if(r.ok){ const j=await r.json(); sha=j.sha; } }catch(e){}
                        await fetch(apiBase, { method:"PUT", headers:{ Authorization:"Bearer "+getPat(), "Content-Type":"application/json" }, body: JSON.stringify({ message:"update", content: utf8ToB64(JSON.stringify(nP, null, 2)), branch: GH.branch, sha }) });
                        PROMOTIONS = nP; sessionStorage.removeItem(CACHE_KEY); rebuildIndex(); render(); window.renderPromoAdmin(); alert("반영 완료!");
                    } catch(err) { alert("실패"); }
                };
                reader.readAsArrayBuffer(f);
            };
    ...
    window.renderSalesAdmin = () => {
        const box = $("#salesAdminBox"); if(!box) return;
        box.innerHTML = `<div class="flex justify-between items-center mb-2"><div class="font-black text-indigo-800">🧠 AI 세일즈 가이드</div></div><div class="text-center cursor-pointer group mt-3 bg-white border border-indigo-100 rounded-lg p-3 hover:bg-indigo-600 transition-colors" id="sgUTrig"><div class="font-black text-indigo-600 text-sm group-hover:text-white">세일즈 엑셀 등록</div></div><input type="file" id="sgFil" class="hidden">`;
        $("#sgUTrig").onclick = () => $("#sgFil").click();
        $("#sgFil").onchange = async (e) => {
            const f = e.target.files[0]; if(!f) return;
            const reader = new FileReader();
            reader.onload = async (ev) => {
                ...
                let nG = {}; rows.forEach(r => { const c = String(r["품번"]||r["상품코드"]||"").trim(); if(c) nG[c] = { keywords: (r["키워드"]||"").split(',').map(k=>k.trim()).filter(Boolean), features: r["특징"]||"", target: r["추천고객"]||"", pitch: r["응대멘트"]||"" }; });
                try {
                    const apiBase = `https://api.github.com/repos/${GH.owner}/${GH.repo}/contents/${SALES_GUIDE_PATH}`;
                    let sha = null; try { const r = await fetch(apiBase+"?t="+Date.now(), {headers:{Authorization:"Bearer "+getPat()}}); if(r.ok){ const j=await r.json(); sha=j.sha; } }catch(e){}
                    await fetch(apiBase, { method:"PUT", headers:{ Authorization:"Bearer "+getPat(), "Content-Type":"application/json" }, body: JSON.stringify({ message:"update", content: utf8ToB64(JSON.stringify(nG, null, 2)), branch: GH.branch, sha }) });
                    SALES_GUIDES = nG; sessionStorage.removeItem(CACHE_KEY); rebuildIndex(); render(); alert("등록 완료!");
                } catch(err) { alert("실패"); }
            };
            reader.readAsArrayBuffer(f);
        };
