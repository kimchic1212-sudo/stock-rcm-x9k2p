...
function rebuildIndex(){
  ...
  for(const r of RAW){
    ...
    if(!map.has(code)){
      map.set(code, {
        품번:code, 품명:r["품명"]||"",
        카테고리:r["카테고리2"]||r["카테고리"]||"",
        분류:r["품목소분류"]||"", 브랜드:brandFromSubcat(r["품목소분류"]||""),
  ...
  BRANDS = Array.from(new Set(PRODUCTS.map(p=>p.브랜드).filter(Boolean))).sort((a,b)=>{
    return PRODUCTS.filter(p=>p.브랜드===b).length - PRODUCTS.filter(p=>p.브랜드===a).length;
  });
  populateBrandChips();
...
function applyFilters(){
  ...
  filtered = PRODUCTS.filter(p=>{
    ...
    if(tokens.length){
      const hay = [p.품번,p.품명,p.카테고리,p.브랜드,p.분류,p.성별,...p.sizes.map(s=>String(s.size))].join(" ").toLowerCase();
...
function card(p){
  ...
  if(p.busanTotal>0){
    ...
    else stockBadge = '<span class="badge badge-stock">● 부산 '+p.busanTotal+' EA</span>';
  } else stockBadge = p.canRequest
    ? '<span class="badge badge-warn">● 부산 품절 · 요청 가능</span>'
  ...
  top.innerHTML =
    ...
    + '</div>'
    + '<div class="shrink-0">'+stockBadge+'</div>';
  el.appendChild(top);
  ...
  title.innerHTML =
    ...
    +   '<span class="font-extrabold text-[17px] sm:text-lg leading-tight" style="color:var(--ink);">'+escapeHtml(p.품명||"-")+'</span>'
    +   '<span class="mono text-[12px] text-[color:var(--muted)] leading-tight truncate" title="'+escapeHtml(p.품번)+'">'+escapeHtml(p.품번)+'</span>'
    + '</div>';
...
function openDetail(p){
  ...
  $("#detailBody").innerHTML = totalsRow + '<div class="hairline overflow-auto"><table class="w-full"><thead>'+head+'</thead><tbody>'+rows+'</tbody></table></div>' + actionTip;
...
function handleFile(file){
  ...
  reader.onload = async (e)=>{
    try{
      ...
      msg.textContent="✓ 업로드 완료 · "+fmt(rows.length)+"행 · "+stamp+"  (직원 화면 반영 1~2분)";
...
function buildDemo(){
  const out = [];
  const items = [
    {분류:"신발(온러닝)", 품명:"CLOUDMONSTER 3", 품번:"ON-CM3-M", cat:"신발", price:259000, sex:"남성", sizes:[260,265,270,275,280,285,290], lastSize:270},
  ...
  items.forEach(s=>{
    s.sizes.forEach(sz=>{
      ...
      const busan = s.품번.includes("CM3-M") ? 0 : (isLast ? 1 : Math.floor(Math.random()*4));
      out.push({
        "품목소분류":s.분류, "품명":s.품명, "품번":s.품번, "카테고리2":s.cat,
