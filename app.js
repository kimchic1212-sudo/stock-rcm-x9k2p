...
function card(p){
  ...
  const shopLink = p.shopNo
    ? '<a class="shop-link" href="https://racement.co.kr/product-detail?productNo='+encodeURIComponent(p.shopNo)+'" target="_blank" rel="noopener" title="Racement 자사몰" onclick="event.stopPropagation()"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>자사몰</a>'
    : '';

  // 상단: 작은 배지들 한 줄 (재고 배지 제거 - 하단 loc bar로 충분)
  const top = document.createElement("div");
  top.className = "top-badges";
  top.innerHTML =
  ...
  const sideImg = (typeof IMAGES !== "undefined" && p.shopNo && IMAGES[p.shopNo])
    ? '<img class="card-thumb-side" src="'+escapeHtml(IMAGES[p.shopNo])+'" alt="'+escapeHtml(p.품명||"")+'" loading="lazy" onerror="this.remove()">'
    : '';
  titleRow.innerHTML =
      '<div class="min-w-0 flex-1 flex flex-wrap items-start gap-1.5 self-center">'
