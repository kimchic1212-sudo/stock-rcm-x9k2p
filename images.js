...
//
// ※ 의존성 (app.js에서 제공):
//   GH, getPat(), ghGet(), ghPut(), utf8ToB64(), escapeHtml(), render(), PRODUCTS
...
async function commitImagesToGitHub(images, meta){
  ...
  let sha = null;
  try{
    // 캐시 버스팅 — 연속 이미지 등록 시 409 Conflict 방지
...
// [FIX #3] syncImages: app.js의 runImageSync()가 호출하는 콜백 인터페이스를 올바르게 구현.
// 수동 등록 모드에서는 자동 fetch를 하지 않으므로 "skip" 페이즈를 즉시 반환.
async function syncImages(products, onProgress){
  if(typeof onProgress === "function"){
    onProgress({ phase:"skip" });
  }
...
// [FIX #4] window._rcmImagesReady 프로미스를 노출해 app.js의 renderAfterImages()가 await 가능하게 함.
// 이미지 로드 완료 후 render()를 직접 호출하지 않음 — loadData()가 이미지 완료를 기다렸다가 렌더링함.
window._rcmImagesReady = loadImages().then(() => {
  refreshManualImageList();
});
